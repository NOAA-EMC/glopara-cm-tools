...
do
 ...
 # Hera or all
 #-----------------------------------------------------
 if [ $machs = hera -o $machs = all ]; then
  ...
  do
    ...
    do
      if [ -d ${DMPDIR}/${dump}.${PDY} ]; then
        ${DMPCPCMD} ${DMPDIR}/${dump}.${PDY} ${ACCOUNT_HERA}@dtn-hera.fairmont.rdhpcs.noaa.gov:${DMPDIR_HERA}/
      fi
    done # dump
  done # CDUMPS
  ...
  do
    herafile=$MONDIR/filecounts.${network}.hera.html
    rsync -av ${ACCOUNT_HERA}@dtn-hera.fairmont.rdhpcs.noaa.gov:/scratch1/NCEPDEV/global/glopara/dump_archive/counts.${network}.hera.html $herafile
  done
